#!/usr/bin/env python3
"""
Script to remove the 'id' column from beers datasets
"""
...
def remove_id_column_from_beers():
    """Remove the id column from all beers splits"""
    output_dir = Path(
    ...
    for split_num in range(1, 4):
        ...
        if "id" in clean_df.columns:
            ...
            logger.info("  Removed 'id' column from clean.csv")
        ...
        # Remove id column if it exists
        if "id" in dirty_df.columns:
            dirty_df = dirty_df.drop("id", axis=1)
            logger.info("  Removed 'id' column from dirty.csv")
...
def main():
    logger.info("Removing 'id' column from beers datasets...")
    remove_id_column_from_beers()
    logger.info("Done!")

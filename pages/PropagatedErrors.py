            ...
            try:
                ...
                if cfg.get("propagation_results"):
                    ...
                    if (
                        ...
                        and results[-1].get("propagation_results")
                    ):
                        st.session_state.propagation_results = results[-1][
                            "propagation_results"
                        ]
                        loaded_from_config = True
                        st.session_state.propagation_saved = True
    ...
    with st.expander(cell_desc):
        ...
        if labeled_cell.get("propagated_cells"):
            ...
            for prop in labeled_cell["propagated_cells"]:
                ...
                prop_info = []
                prop_info.append(
                    f"- \U0001f539 Table: `{prop.get('table', 'Unknown')}`"
                )
...
if (
    ...
    if os.path.exists(cfg_path):
        ...
        if (
            cfg["results"]
            and cfg["results"][-1].get("Time", "").split(" ")[0]

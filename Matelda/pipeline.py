...
from Matelda.marshmallow_pipeline.column_grouping_module.grouping_columns import (
    column_grouping,
)
from Matelda.marshmallow_pipeline.error_detection import error_detector
from Matelda.marshmallow_pipeline.table_grouping_module.grouping_tables import (
    table_grouping,
)
from Matelda.marshmallow_pipeline.utils.loading_results import (
    loading_columns_grouping_results,
)
from Matelda.marshmallow_pipeline.utils.saving_results import get_all_results
...
def main(execution):
    ...
    save_mediate_res_on_disk = bool(
        int(configs["EXPERIMENTS"]["save_mediate_res_on_disk"])
    )
    ...
    raha_config["error_detection_algorithms"] = configs["RAHA"][
    ...
    table_grouping_res_available = bool(
        int(configs["TABLE_GROUPING"]["tg_res_available"])
    )
    ...
    column_grouping_res_available = bool(
        int(configs["COLUMN_GROUPING"]["cg_res_available"])
    )
    ...
    cell_clustering_res_available = bool(
        int(configs["CELL_GROUPING"]["cell_clustering_res_available"])
    )
    ...
    if save_mediate_res_on_disk:
        with open(
            os.path.join(experiment_output_path, "tables_dict.pickle"), "wb+"
        ) as handle:
    ...
    if table_grouping_enabled:
        ...
        if save_mediate_res_on_disk:
            with open(
                os.path.join(experiment_output_path, "table_group_dict.pickle"), "wb+"
            ) as handle:
                pickle.dump(table_grouping_dict, handle)

    logging.info("Table grouping is done")
    logging.info(
        "I need at least 2 labeled cells per table group to work at all and at least 2 * 6 labeled cells per table group to work effectively! Thant means you need to label {} cells if you want reasonable (!) results:".format(
            2 * 6 * len(table_grouping_dict)
        )
    )
    print(
        "I need at least 2 labeled cells per table group to work at all and at least 2 * 6 labeled cells per table group to work effectively! Thant means you need to label {} cells if you want reasonable (!) results:".format(
    ...
    with open(
        os.path.join(final_results_path, "y_local_cell_ids.pickle"), "wb+"
    ) as handle:
        pickle.dump(y_local_cell_ids, handle)
    with open(
        os.path.join(final_results_path, "predicted_all.pickle"), "wb+"
    ) as handle:
        pickle.dump(predicted_all, handle)
    with open(
        os.path.join(final_results_path, "y_labeled_by_user_all.pickle"), "wb+"
    ) as handle:
        pickle.dump(y_labeled_by_user_all, handle)
    with open(
        os.path.join(final_results_path, "unique_cells_local_index_collection.pickle"),
        "wb+",
    ) as handle:

...
def get_classification_results(
    ...
    results_dir,
    samples,
    unique_cells_local_ids_collection,
):
    ...
    try:
        for i in predicted_all.keys():
            ...
            swapped_cell_local_ids = {
                v: (k[0], k[1], k[2]) for k, v in cell_local_ids.items()
            }
            ...
            with open(
                os.path.join(
                    results_dir,
                    "scores_col_cluster_" + str(i[0]) + "_" + str(i[1]) + ".pickle",
                ),
                "wb",
            ) as file:
...
def process_cell_cell_results(
    cell_key,
    key,
    unique_cells_local_index_collection,
    y_local_cell_ids,
    y_test_all,
    col_cluster_prediction,
    all_tables_dict,
    samples,
):
    try:
...
def process_col_group_cell_results(
    key,
    unique_cells_local_index_collection,
    y_local_cell_ids,
    y_test_all,
...
def process_batches(
    ...
    for key in batch:
        all_results.extend(
            process_col_group_cell_results(
                key,
                unique_cells_local_index_collection,
                y_local_cell_ids,
                y_test_all,
...
def get_tables_dict(init_tables_dict, sandbox_path, dirty_file_names, clean_file_names):
    ...
    for table in table_dirs:
        if not table.startswith("."):
            ...
            table_df = read_csv(
                os.path.join(table_path, dirty_file_names),
                low_memory=False,
                data_type="str",
            )
...
def get_all_results(
    ...
    tables_dict = get_tables_dict(
        init_tables_dict, tables_path, dirty_file_names, clean_file_names
    )
    if final_result_df:
...
def get_all_results_from_disk(
    output_path, tables_path, dirty_file_names, clean_file_names
):
    ...
    with open(
        os.path.join(output_path, "results", "final_results", "y_test_all.pickle"), "rb"
    ) as file:
        y_test_all = pickle.load(file)
    with open(
        os.path.join(
            output_path, "results", "final_results", "y_local_cell_ids.pickle"
        ),
        "rb",
    ) as file:
        y_local_cell_ids = pickle.load(file)
    with open(
        os.path.join(output_path, "results", "final_results", "predicted_all.pickle"),
        "rb",
    ) as file:
        predicted_all = pickle.load(file)
    with open(
        os.path.join(
            output_path, "results", "final_results", "y_labeled_by_user_all.pickle"
        ),
        "rb",
    ) as file:
    ...
    with open(
        os.path.join(
            ...
            "results",
            "final_results",
            "unique_cells_local_index_collection.pickle",
        ),
        "rb",
    ) as file:
        unique_cells_local_index_collection = pickle.load(file)
    with open(
        os.path.join(output_path, "results", "final_results", "samples.pickle"), "rb"
    ) as file:

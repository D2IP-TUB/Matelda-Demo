...
from Matelda.marshmallow_pipeline.column_grouping_module.col_grouping import (
    col_grouping,
)
from Matelda.marshmallow_pipeline.utils.read_data import read_csv


def get_n_col_groups(
    table_grouping_dict,
    table_size_dict,
    labeling_budget,
    ...
    for table_group in table_grouping_dict:
        ...
        tg_stats[table_group] = {
            "n_cols": n_cols_in_tg,
            "n_cells": n_cells_in_tg,
            "max_n_col_groups": 1,
        }
    for table_group in table_grouping_dict:
        remained_labeling_budget = labeling_budget - 2 * labels_per_cell_group * sum(
            item["max_n_col_groups"] for item in tg_stats.values()
        )
        max_n_col_groups = min(
            tg_stats[table_group]["n_cols"],
            math.floor(
                remained_labeling_budget
                * tg_stats[table_group]["n_cols"]
                / count_all_cols
                / 2
                / labels_per_cell_group
            ),
        )  # 2 is the minimum number of labels for each column group
        tg_stats[table_group]["max_n_col_groups"] += max_n_col_groups
    tg_stats = dict(
        sorted(
            tg_stats.items(), key=lambda item: item[1]["max_n_col_groups"], reverse=True
        )
    )
    processed = 0
    i = 0
    while sum(
        item["max_n_col_groups"] for item in tg_stats.values()
    ) < labeling_budget / 2 / labels_per_cell_group and processed < len(
        table_grouping_dict
    ):
    ...
    logging.info(
        "Labeling Budget: %s, N Col Groups: %s", labeling_budget, total_n_col_groups
    )
...
def column_grouping(
    ...
    for table_group in table_grouping_dict:
        ...
        for table in table_grouping_dict[table_group]:
            df = read_csv(
                os.path.join(path, table), low_memory=False, data_type="default"
            )
        ...
        pool.apply(
            ...
            args=(
                table_group,
                cols,
                tg_stats[table_group]["max_n_col_groups"],
                mediate_files_path,
                cg_enabled,
                col_grouping_alg,
                n_cores,

...
import pandas as pd
from Matelda.marshmallow_pipeline.cell_grouping_module.cell_folding import (
    cell_cluster_sampling_labeling,
    cluster_column_group,
)
from Matelda.marshmallow_pipeline.cell_grouping_module.extract_table_group_charset import (
    extract_charset,
)
from Matelda.marshmallow_pipeline.cell_grouping_module.generate_cell_features import (
    get_cells_features,
)
from Matelda.marshmallow_pipeline.cell_grouping_module.sampling_labeling import (
    get_n_labels,
    update_n_labels,
)
...
def error_detector(
    ...
    if cell_feature_generator_enabled:
        ...
        logging.info(
            "Generating cell features disabled, loading from previous results from disk"
        )
        with open(os.path.join(output_path, "features.pickle"), "rb") as pickle_file:
            features_dict = pickle.load(pickle_file)
        with open(
            os.path.join(output_path, "tables_tuples.pickle"), "rb"
        ) as pickle_file:
    ...
    for result in results:
        if result is not None:
            ...
            unique_cells_local_index_collection.update(
                result["unique_cells_local_index_collection"]
            )
            ...
            logging.info(
                "Len selected_samples: %s", str(len(result["selected_samples"]))
            )
            logging.info(
                "Number of Labeled Cells (user): %s",
                str(result["n_user_labeled_cells"]),
            )

    end_time = time.time()
    logging.info(
        "Sampling and labeling clusters took %s seconds", str(end_time - start_time)
    )
    logging.info("Saving results")
    if save_mediate_res_on_disk:
        with open(
            os.path.join(output_path, "original_data_keys.pkl"), "wb"
        ) as filehandler:
            pickle.dump(original_data_keys, filehandler)

        with open(
            os.path.join(results_path, "sampled_tuples.pkl"), "wb"
        ) as filehandler:
...
def test(
    ...
    logging.info(
        "Starting test; Column cluster: %s; Table cluster %s",
        col_cluster,
        table_cluster,
    )
    original_data_keys = []
    ...
    if X_labeled_by_user is not None:
        ...
        X_labeled_by_user_all[(str(table_cluster), str(col_cluster))] = (
            X_labeled_by_user
        )
        y_labeled_by_user_all[(str(table_cluster), str(col_cluster))] = (
            cell_cluster_sampling_labeling_dict["y_labeled_by_user"]
        )

        predicted_all[(str(table_cluster), str(col_cluster))] = (
            cell_cluster_sampling_labeling_dict["predicted"]
        )
        y_test_all[(str(table_cluster), str(col_cluster))] = (
            cell_cluster_sampling_labeling_dict["y_test"]
        )
        y_local_cell_ids[(str(table_cluster), str(col_cluster))] = (
            cell_cluster_sampling_labeling_dict["y_cell_ids"]
        )
        unique_cells_local_index_collection[(str(table_cluster), str(col_cluster))] = (
            cell_cluster_sampling_labeling_dict["datacells_uids"]
        )

    init_labels_tg_cg = df_n_labels[
        (df_n_labels["table_cluster"] == table_cluster)
        & (df_n_labels["col_cluster"] == col_cluster)
    ]["n_labels"].values[0]

    logging.info(
        "Done test; Column cluster: %s; Table cluster %s; Used labels %s , Init labels: %s",
        col_cluster,
        table_cluster,
        str(len(X_labeled_by_user) if X_labeled_by_user is not None else 0),
        init_labels_tg_cg,
    )

    return {
        "original_data_keys": original_data_keys,

...
import numpy as np
from Matelda.marshmallow_pipeline.table_grouping_module.table_grouping_bert import (
    group_tables,
)
...
def table_grouping(
    aggregated_lake_path: str,
    output_path: str,
    table_grouping_method: str,
    ...
    if table_grouping_method == "santos":
        ...
        with open(
            "/home/fatemeh/VLDB-Jan/ED-Scale-Dev/ED-Scale/output_DGov_NTR_santos/g_santos.pickle",
            "rb",
        ) as handle:
            g_santos = pickle.load(handle)

        with open(
            "/home/fatemeh/VLDB-Jan/ED-Scale-Dev/ED-Scale/output_DGov_NTR_santos/table_size_dict.pickle",
            "rb",
        ) as handle:
        ...
        )  # Use precomputed since we provide a distance matrix
        ...
        table_group_dict, table_size_dict = group_tables(
            aggregated_lake_path, batch_size=5, pool=pool
        )

    if save_mediate_res_on_disk:
        with open(
            os.path.join(output_path, "table_group_dict.pickle"), "wb+"
        ) as handle:
...
def run_santos(aggregated_lake_path: str, output_path: str):
    ...
    g_santos, table_size_dict = marshmallow_pipeline.santos.codes.query_santos.main(
        1, 3
    )

...
def get_train_test_sets_per_col(
    X_temp, y_temp, samples_dict, cell_clustering_df, uids, output_path
):
    logging.debug("Train-Test set preparation")
    cells_per_cluster = cell_clustering_df["cells_per_cluster"].values[0]
    samples_df = pd.DataFrame(samples_dict)
    ...
    for key in clusters:
        try:
            ...
            logging.error("Error in get_train_test_sets: %s", e)
    logging.debug(
        "*******Time for train-test set preparation: %s", time.time() - s_time
    )
    s_time = time.time()
    logging.debug("Start classification Per Column")
    for col in X_train_cols:
        gbc, predicted_cols[col] = classify(
            X_train_cols[col], y_train_cols[col], X_test_cols[col]
        )
        if gbc is not None:

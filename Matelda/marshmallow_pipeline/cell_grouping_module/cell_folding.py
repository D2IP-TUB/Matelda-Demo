...
from Matelda.marshmallow_pipeline.classification_module.classifier import classify
from Matelda.marshmallow_pipeline.classification_module.get_train_test import (
    get_train_test_sets,
...
def cell_cluster_sampling_labeling(
    cell_clustering_df,
    ...
    try:
        if cell_clustering_df["n_labels_updated"].values[0] > 0:
            ...
            samples_dict, cell_clustering_df, n_user_labeled_cells = sampling(
                cell_clustering_df,
                X_temp,
                y_temp,
                value_temp,
        ...
        if samples_dict is None:
            ...
            if classification_mode == 0:
                ...
                logging.info(
                    "start classification for cluster %s",
                    str(cell_clustering_df["col_cluster"].values[0]),
                )
                ...
                X_train, y_train, X_test, y_test, y_cell_ids, predicted = (
                    get_train_test_sets_per_col(
                        ...
                        samples_dict,
                        cell_clustering_df,
                        cell_cluster_cells_dict["datacells_uids"],
                        output_path,
                    )
                )
...
def cluster_column_group(
    ...
    with open(os.path.join(col_groups_dir, file_name), "rb") as file:
        ...
        table_cluster = int(
            file_name.removeprefix("col_df_labels_cluster_").removesuffix(".pickle")
        )
        ...
        for _, col_cluster in enumerate(clusters):
            ...
            cell_cluster_cells_dict_all[table_cluster][col_cluster] = (
                cell_cluster_cells_dict
            )
    ...
    return {
        "table_cluster": table_clusters,
        "col_clusters": col_clusters,
        "cell_cluster_cells_dict_all": cell_cluster_cells_dict_all,
        "cell_clustering_dict_all": cell_clustering_dict_all,
    }

...
def generate_features(self, d):
    ...
    for j in range(d.dataframe.shape[1]):
        ...
        for strategy_profile in d.strategy_profiles:
            ...
            if strategy[0] == "PVD":
                if strategy[1][0] == d.dataframe.columns[j]:
                    ...
                    strategy_profile_dict_all_without_rvd[str(strategy)] = (
                        strategy_profile
                    )
        ...
        try:
            for key in sorted_keys:
                if key[0] == "RVD_orig":
                    ...
                    strategy_profile_output = strategy_profiles_dict_rvd_orig[str(key)][
                        "output"
                    ]
                    strategy_profile_outputted_cells = strategy_profiles_dict_rvd_orig[
                        str(key)
                    ]["outputted_cells"]
                    ...
                    sp_output = strategy_profile_dict_all_without_rvd[str(key)][
                        "output"
                    ]
        ...
        for row_idx in RVD_orig_outputs:
            ...
            insert_idx = idx + get_bucket(
                LRVD_violations / n_rvd_rules_left if n_rvd_rules_left > 0 else 0
            )
            feature_vectors[row_idx, insert_idx] = 1
            ...
            insert_idx = idx + get_bucket(
                RRVD_violations / n_rvd_rules_right if n_rvd_rules_right > 0 else 0
            )
            feature_vectors[row_idx, insert_idx] = 1

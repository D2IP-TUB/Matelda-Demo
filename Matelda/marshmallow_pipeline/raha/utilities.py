...
def get_tuple_wise_evaluation(d, correction_dictionary):
    ...
    actual_dirty_tuples = {
        ...
        if int(
            sum(
                [
                    (i, j) in actual_errors_dictionary
...
def dataset_profiler(dataset_dictionary):
    ...
    d.results_folder = os.path.join(
        os.path.dirname(dataset_dictionary["path"]), "raha-baran-results-" + d.name
    )
    ...
    for attribute in d.dataframe.columns.tolist():
        ...
        column_profile = {
            "characters": {
                ch: characters_dictionary[ch] / d.dataframe.shape[0]
                for ch in characters_dictionary
            },
            "values": {
                v: values_dictionary[v] / d.dataframe.shape[0]
                for v in values_dictionary
            },
        }
        pickle.dump(
            column_profile,
            open(os.path.join(dp_folder_path, attribute + ".dictionary"), "wb"),
        )
...
def evaluation_profiler(dataset_dictionary):
    ...
    for strategy_file in strategies_file_list:
        ...
        for column_index, attribute in enumerate(d.dataframe.columns.tolist()):
            actual_column_errors = {
                (i, j): 1 for (i, j) in actual_errors_dictionary if j == column_index
            }
            detected_column_cells = [
                (i, j) for (i, j) in strategy_output if j == column_index
            ]
            tp = 0.0
    ...
    for j, attribute in enumerate(d.dataframe.columns.tolist()):
        pickle.dump(
            columns_performance[j],
            open(os.path.join(ep_folder_path, attribute + ".dictionary"), "wb"),
        )


def get_selected_strategies_via_historical_data(
    dataset_dictionary, historical_dataset_dictionaries
):
    """
        ...
        for hdd in historical_dataset_dictionaries:
            if hdd["name"] != d.name:
                ...
                for hci, ha in enumerate(hd.dataframe.columns.tolist()):
                    hdp_folder_path = os.path.join(
                        os.path.dirname(hdd["path"]),
                        "raha-baran-results-" + hdd["name"],
                    )
                    hcp = pickle.load(
                        open(
                            os.path.join(
                                hdp_folder_path, "dataset-profiling", ha + ".dictionary"
                            ),
                            "rb",
                        )
                    )
                    ...
                    for k in list(set(ncp["characters"]) | set(hcp["characters"])):
                        nfv.append(ncp["characters"][k]) if k in ncp[
                            "characters"
                        ] else nfv.append(0.0)
                        hfv.append(hcp["characters"][k]) if k in hcp[
                            "characters"
                        ] else hfv.append(0.0)
                    for k in list(set(ncp["values"]) | set(hcp["values"])):
                        nfv.append(ncp["values"][k]) if k in ncp[
                            "values"
                        ] else nfv.append(0.0)
                        hfv.append(hcp["values"][k]) if k in hcp[
                            "values"
                        ] else hfv.append(0.0)
    ...
    for hdd in historical_dataset_dictionaries:
        if hdd["name"] != d.name:
            hd = raha.dataset.Dataset(hdd)
            for hci, ha in enumerate(hd.dataframe.columns.tolist()):
                ep_folder_path = os.path.join(
                    os.path.dirname(hdd["path"]),
                    "raha-baran-results-" + hdd["name"],
                    "evaluation-profiling",
                )
                strategies_performance = pickle.load(
                    open(os.path.join(ep_folder_path, ha + ".dictionary"), "rb")
                )
                if (hd.name, ha) not in f1_measure:
                    f1_measure[(hd.name, ha)] = {}
                for strategy_name in strategies_performance:
                    f1_measure[(hd.name, ha)][strategy_name] = strategies_performance[
                        strategy_name
                    ][2]
                ...
                for hci, ha in enumerate(hd.dataframe.columns.tolist()):
                    ...
                    for strategy_name in f1_measure[(hd.name, ha)]:
                        ...
                        if sn[0] == "OD" or sn[0] == "KBVD":
                            if (
                                strategy_name not in strategies_score[na]
                                or score >= strategies_score[na][strategy_name]
                            ):
                            ...
                            sn[1][0] = na
                            if (
                                json.dumps(sn) not in strategies_score[na]
                                or score >= strategies_score[na][json.dumps(sn)]
                            ):
                            ...
                            for aa in d.dataframe.columns.tolist():
                                if (
                                    aa != na
                                    and columns_similarity[
                                        (d.name, aa, hd.name, that_a)
                                    ]
                                    > most_similar_v
                                ):
                                    most_similar_v = columns_similarity[
                                        (d.name, aa, hd.name, that_a)
                                    ]
                                    most_similar_a = aa
                            sn[1][this_a_i] = na
                            ...
                            sys.stderr.write(
                                "I do not know this error detection tool!\n"
                            )
    sp_folder_path = os.path.join(d.results_folder, "strategy-profiling")
    strategies_output = {}
    strategies_runtime = {}
    ...
    for a in d.dataframe.columns.tolist():
        sorted_strategies = sorted(
            strategies_score[a].items(), key=operator.itemgetter(1), reverse=True
        )
        good_strategies = {}
        previous_score = 0.0
        for sn, ss in sorted_strategies:
            ...
            second_sum = sum(
                [
                    math.fabs(good_strategies[s_1] - good_strategies[s_2])
                    for s_1, s_2 in itertools.product(
                        good_strategies.keys(), good_strategies.keys()
                    )
                    if s_1 > s_2
                ]
            )
        ...
        for sn in good_strategies:
            ...
            strategy_profile = {
                ...
                "output": [
                    cell
                    for cell in strategies_output[sn]
                    if d.dataframe.columns.tolist()[cell[1]] == a
                ],
                "runtime": runtime,
                "score": good_strategies[sn],
                "new_column": strategies_anchor[a][sn][0]
                + "."
                + strategies_anchor[a][sn][1],
                "historical_column": strategies_anchor[a][sn][2]
                + "."
                + strategies_anchor[a][sn][3],
            }
...
def get_selected_strategies_via_ground_truth(dataset_dictionary, strategies_count):
    ...
    for nci, na in enumerate(d.dataframe.columns.tolist()):
        strategies_performance = pickle.load(
            open(os.path.join(ep_folder_path, na + ".dictionary"), "rb")
        )
        for strategy_name in strategies_performance:
    ...
    random_strategies = {
        ...
        for s, f1 in [
            sorted_f1_measure[i]
            for i in random.sample(range(len(sorted_f1_measure)), strategies_count)
        ]
    }
...
def error_detection_with_selected_strategies(
    dataset_dictionary, strategy_profiles_list
):
    """
